(function (global, factory) {
	...
	(global = global || self, global.EasyPopover = factory());
	...
	*/
	...
	var FORCED = descriptors && isForced_1('RegExp', (!CORRECT_NEW || fails(function () {
	  re2[MATCH$1] = false;
	...
	var EasyPopover = {
	  ...
	  init: function init() {
	    ...
	    var closeSelector = this.options.closeSelector;
	  ...
	  handleEvent: function handleEvent(scopeElm, selector, type, func) {
	    var isRemove = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : false;
	    var listener = {
	      handleEvent: func,
	      scopeElm: scopeElm,
	      selector: selector,
	  ...
	  openAll: function openAll(event) {
	    var scopeElm = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : null;
	    ...
	    var els = scopeElm.querySelectorAll(selector);
	  ...
	  open: function open(el) {
	    var activeClass = el.dataset.active_class != null ? el.dataset.active_class : 'is-active';
	    var name = el.dataset.name != null ? el.dataset.name : 'popover';
	    var strageName = name + '_isDeleted';
	    var container = el.dataset.container != null ? el.dataset.container : '';
	    var targetParams = el.dataset.targetParams != null ? el.dataset.targetParams.split(',') : [];
	    var targetParamName = el.dataset.targetParamName != null ? el.dataset.targetParamName : '';
	    var expiredDate = el.dataset.expire != null ? el.dataset.expire : '';
	    ...
	    if (arrowStyle) {
	      var arrowEl = el.querySelector('.popover-arrow');
	    ...
	    if (content) {
	      var bodyEl = el.querySelector('.popover-body');
	  ...
	  close: function close(event) {
	    ...
	    var parentSelector = $closeBtn.dataset.parentSelector != null ? $closeBtn.dataset.parentSelector : '.js-popover';
	    var $popover = EasyPopover.closest($closeBtn, parentSelector, $scope);
	    if ($popover == null) return;
	    var activeClass = $popover.dataset.active_class != null ? $popover.dataset.active_class : 'is-active';
	    var name = $popover.dataset.name != null ? $popover.dataset.name : 'popover';
	    var strageName = name + '_isDeleted';
	  ...
	  closest: function closest(node, searchSelector, scopeElm) {
	  ...
	  isExpired: function isExpired(dateStr) {
	    var expiredTime = new Date("".concat(dateStr, " 00:00")).getTime() + 1000 * 60 * 60 * 24;
